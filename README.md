...
```shell
# Создание или поднятие Docker-контейнера с PostgreSQL (если нужно без Docker, то смотри мануал в отдельной главе)
cargo up
```
```shell
...
```
```shell
# Запуск сервера (БД не затрагивает)
cargo run
```
```shell
# Обнуление табицы и создание новых записей (100 шт.)
cargo reset
```
